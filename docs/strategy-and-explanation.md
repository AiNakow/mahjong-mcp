...
总进张相同的候选仍可能有明显优劣。尤其在六搭子或超块手牌中，策略不应只看切后总进张，而要判断哪个候选更好地压缩结构、释放牌位。

该因素属于 `shapeScore`，可称为 `blockEfficiency`。它处理的问题是：

```text
同样进张下，哪个结构用更少牌位完成同等功能，哪个结构更值得保留。
```
...
```text
30899p2477888s11z
= 3p 0p 8p 9p9p 2s4s 7s7s8s8s8s 1z1z
```

切 `8p` 与切 `2s` 的总进张相同，但 `24s` 是独立坎张搭子，占 2 张只接受 `3s`；`899p` 是压缩复合块，占 3 张同时保留 `99p` 对子、`78p` 顺子变化、`999p` 刻子/对对路线和七对子路线。六搭子时，应优先拆 `24s` 这类弱独立搭子，而不是拆 `899p` 这类压缩块。

该判断必须考虑实际剩余枚数，而不是固定形状表：

```text
weakTaatsuValue =
  actualImprovementTiles
  * shapeWeight
  * routeContribution

compressedBlockValue =
  actualImprovementTiles
  + pairValue
  + routeContribution
```
...
- 若候选切牌拆掉的是孤立坎张或边张，且该搭子的实际进张枚数不高于被保留压缩块的实际变化枚数，则给 `blockEfficiency` 加分。
- 若候选切牌拆掉的是 `899`、`112`、`1123`、`7889`、`6778` 等压缩复合块，且手牌仍处于六搭子或超块状态，则扣分。
- 压缩复合块包括对子邻接/嵌张块（如 `899`、`799`）、连续四连形（如 `4567`）和刻子旁张块（如 `3555`、`6667`）；通用形状分不直接放大这些块，主要在六搭子/超块比较中生效。
- 若弱搭子的核心进张已经大量可见，例如 `24s` 的 `3s` 已见 3 枚，则加分更高。
- 若复合块相关进张大量可见，或复合块没有对子/路线贡献，则保留价值下降。
- 宝牌、赤宝、役牌对子和明确路线可修正块价值，但不应改变实际剩余枚数。

这类分数只在候选总进张接近时起主要作用，不能覆盖明确向听优势或明显进张差。

### 搭子超载与预计听牌质量

搭子超载时，不能只按当前搭子种类排序。需要估算“这个搭子如果被保留到听牌，最终更可能听什么”，再决定拆哪一组。

当前实现把该因素作为 `shapeScore` 内的 `blockEfficiency` 子项处理：

```text
projectedTaatsuValue =
  actualWaitTiles * currentShapeWeight
  + finalWaitRonBonus
  + directImprovementBonus
  + doraOrRouteAdjustment
```

含义：

- `actualWaitTiles`：该搭子的直接进张剩余枚数。
- `currentShapeWeight`：两面、嵌张、边张的基础完成率权重。
- `finalWaitRonBonus`：两面最终听牌的出易度修正。无断幺等路线约束时，`23` 这类可转外侧听牌的两面通常优于 `45` 这类中张两面；若整手强烈偏断幺，则终端待牌奖励会被折减。
- `directImprovementBonus`：嵌张的直接改良。`79` 虽然是嵌张，但摸 `6` 可转 `67` 两面，因此通常优于纯边张 `12`；`35` 因为有 `2/6` 两侧改良且中张完成质量更好，通常优于 `79`。
- `doraOrRouteAdjustment`：宝牌、赤宝、路线关键牌可以修正，但不能伪造剩余枚数。
...
无额外路线约束的两面比较：23 > 45
嵌张/边张比较：35 > 79 > 12/89
强断幺或三色/一气/染手/宝牌路线存在时，按路线价值修正
```

因此六搭子或超载时：

- 拆掉 `12`、`89` 这类没有直接两面改良的纯边张会获得正面 `blockEfficiency`。
- 拆掉 `79` 这类仍有直接两面改良的嵌张可能被扣分，尤其旁边没有更差搭子时。
- 拆掉 `23` 这类预计最终外侧两面的搭子，不应因为包含 `2` 就机械视为弱于 `45`。

### 副露速度价值

副露价值也不应混入静态打点。它描述的是“保留这个形状后，更容易从他家弃牌推进”的速度收益，当前作为 `shapeScore` 内的 `callability` 子项输出。
...
- 碰牌的 `callSourceMultiplier` 高于吃牌。吃只能吃上家，碰可以碰三家，因此对子可副露速度价值通常显著高于同等枚数的吃牌搭子。
- 幺九对子形状价值不高，但在已有役、役牌对子、断幺以外的确定路线、已副露成役、终盘形式听牌等场景下，因出牌倾向高且可碰三家，速度价值可以高于中张对子。
- 役牌对子仍是最高优先的可碰资源，因为它同时提供速度和成役。
- 无役副露或副露后役不稳定时，`yakuCertainty` 会压低普通幺九对子和吃牌搭子的速度奖励。
- 宝牌、宝牌周边和危险关键牌的 `discardLikelihood` 会折减，避免把理论可碰/可吃当成真实容易出现。
...
形状价值：中张对子 > 普通幺九对子
副露速度价值：幺九对子 > 中张对子
综合价值：役牌对子最高；普通幺九对子只有在副露后有役或路线明确时才明显加分
```

解释层会把该信号作为形状理由输出，例如“保留幺九对子，较容易从三家碰出，副露速度价值较高”。该分数仍属于同向听和近似牌效下的排序修正，不应覆盖明确向听优势。
...
- 孤立幺九字牌。
- 六搭子/超块时的块数管理与牌位效率：优先拆实际进张不占优的孤立坎边搭子，保留更压缩的对子复合块。
